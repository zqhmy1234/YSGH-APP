...
def remove_by_name(tpl):
    ...
    for nm in DROP_NAMES:
        ...
        while True:
            ...
            rest = tpl[m.end():]
            depth, idx = 1, 0
            op = re.compile(r'<view\b')
            cl = re.compile(r'</view>')
            while depth > 0:
                no = op.search(rest, idx)
                nc = cl.search(rest, idx)
                if nc is None:
                    raise ValueError('unbalanced ' + nm)
...
def add_tap_by_name(tpl, name, fn):
    ...
    # image 自闭合形式：<!-- icon: 名字 -->
    pat2 = re.compile(r'<image class="(n[0-9_]+)" (src="[^"]*")( @tap="[^"]*")? /> <!-- icon: '
                      + re.escape(name) + ' -->')
    m2 = pat2.search(tpl)
    if m2:
        return (tpl[:m2.start()]
                + f'<image class="{m2.group(1)}" {m2.group(2)} @tap="{fn}" /> <!-- icon: {name} -->'
                + tpl[m2.end():])
...
CONFIG.update({
    'ai': {
        # AI 对话页暂未建独立页面，复用 detail 占位？否——跳过组装
        'gen': 'ai_gen.uvue', 'target': r'pages\\detail\\detail.uvue',
        'skip': True,
    },
    'search': {
