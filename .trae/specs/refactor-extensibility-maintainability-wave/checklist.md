...
- [x] **A7 提请用户拍板 → 三件均已拍板（2026-09-24）**：① `schema.sql` vs ORM 权威 → **ORM 为权威**（`schema.sql` 降级为 CI 建库快照，§5.10 拍板 10.3）；② 64 条功能缺陷 → **另开「功能修复波」**（拍板 10.1）；③ B5 **不再顺延**（编译门已恢复，B5a–B5.2f 均已完成）—— 证据：`docs/决策台账.md` §5.10 拍板结果表 + `refactor-ledger.md` §9.7 C/D；**用户另拍板「L-12 令牌收敛另立令牌波」**（台账 §5.10 拍板 10.4）
...
- [x] ✅ **覆盖率阈值已实测解除（2026-09-25 · Docker/Qdrant 已起）**：`review_agent --full` → **EXIT 0 / 审核通过**，`tests` 段 ✅（pytest 主套件 + `-m rag` 分组 `--cov-append` 合并判定）⇒ **“Required test coverage of 60% reached. Total coverage: 84.59%”**；`api_smoke` ✅、`research` ✅（18 场景全过）；同时 `syntax` 376 文件 ✅、`lint` ✅、无 CRITICAL —— 证据：`.cowork-temp/test-report.json`（`passed=true`、`blocking_sections=[]`、`env_blocked_sections=[]`）
...
- [x] **B5.2f（script 半）已完成（R9–R15 共 7 子步）→ `RecordSheet` 753 / `TabIndex` 738，均 <800 且均销 axis-5 条目**；`favorites.uvue` 基线 974 vs 实际 957 的记账差已随 B5.2f-1 下调修正 —— 证据：`useWaveform`/`useRecordAnimations`/`custom_labels`/`record_ui`/`useVoiceRecord`/`usePhotoAttach`/`useEventOps` 七处抽取，逐子步冷编译 ✅（见台账 §8.6–§8.14）；**运行期验收仍待真机**（不得以冷编译冒充）
...
- [x] 🎯 **L-18/L-19/L-20 样式外置 ⇒ 轴 5 filesize 棘轮清零（第十六轮）**：`TabAi.uvue` 974→**469**（style 507 → `client/styles/tab-ai.css`）/ `favorites.uvue` 957→**500**（style 459 → `favorites.css`）/ `manage.uvue` 891→**466**（style 427 → `portrait-manage.css`）—— 沿用 B5d/B5.2e 已证机制（整块 `<style>` **纯移动** + uvue 侧只留 `@import`），**零 script/运行时改动**；证据：① **等价性**＝`当前 uvue + 还原 css` 与 `git show HEAD:<file>` **归一化后字节全等**（TabAi 工作区 CRLF vs git blob LF，差值恰＝行数）；② **冷编译成功**（67.8s）；③ **产物取证**＝三个 `*.style.bytes` 分别含 `ai-root`/`ai-header`、`page-root`/`nav-title`、`page`/`serif` ⇒ `@import` 被真正处理；④ 三条 baseline 条目删除后 `audit_harness filesize` → **存量超阈 0 个**、无 CRITICAL —— 台账 §8.15
