...
- [ ] 🧭 **B5.2f 剩余 = 设计决策项**（`RecordSheet` −254 / `TabIndex` −247）：剩余全为**表单编排逻辑**（依赖 10+ ref 与 emit/动画实例）⇒ 继续抽**须重新分配状态归属**（方案 A 整块 view-model 进 composable / 方案 B 拆子组件），**非纯移动**且**运行期只有真机可验** ⇒ 依"探索性设计先探讨"规则**提请决策后再做**
