...
- [x] Task B11（第七轮）声明漂移残余：`D05-9` ✅（rerank 模型名单点化）/ `D02-7` ✅（photos 键布局单点化，新 `services/media_keys.py`）/ `D04-3` ✅（`night` 接线 + 删冗余 `gps_speed`）/ `D12-7` ✅（失效证据书面作废）/ `D02-9` ⏸ **登记未改**（属行为变更 → 功能波）/ `D09-10` ✅（wechat status 唯一来源 + 防漂移门禁）。**余**：`D07-14`。见 ledger §8.5。
