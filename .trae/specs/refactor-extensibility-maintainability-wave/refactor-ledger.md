...
>    用户以为删了、云端其实没删）。修法：**权威行存在即实体存在**（归属仍由既有权校验把关）。
> 2. **D08-18（P1 · 新登记，未修）**：REST **创建**内容（`POST /contents`、分片 `complete`、
>    `POST /contents/upload`）**不写 `offline_queue` 变更日志** ⇒ 他端 pull **拉不到新内容**
>    （仅在 `reconcile` 的 `missing_on_client` 里体现，而 pull 无源）。与 **D08-4**（客户端镜像
>    丢弃 `value` ⇒ 字段级收敛不可达）同属"端间一致性"族，建议**合批**处理——否则「双向写穿」
>    只覆盖"改/删"，**创建**这条链仍是断的。

**给功能波的三个必修簇**（按后果排序，均属"MVP 允许缺失但不允许阉割"范畴）：
1. **端云聚合同源（域④）**：D04-1/2 + 夹具覆盖缺口 D04-15 → 双跑门禁当前**无鉴别力**，端云"同源契约"实际已分叉。✅ **2026-09-25 簇① 已收口**（D04-1/2/14/15，并顺带把 AGG-016 从"端侧单跑 + 恒真断言 + 现场算期望"修成"云侧跑冻结快照 + 四分支探针真红"）。
2. **软删 30 天承诺（域⑧）**：D08-1/2/3 → 用户可见路径上"彻底清除"与"离线改可见"两条承诺均未落地，且出现错序。✅ **2026-09-25 簇② 已收口**（D08-1/2/3 + D08-5 顺手修复 + 上面的"D08-1 更深一层"；新增 `services/sync_writes.py` 单一写入语义路径，REST 与 sync 同调）。
