...
| B10-i | 门禁域**剩余项**（**待执行**）：D14-18 四工具退出码口径不一（0/1/2 各异、"环境错误"被吞）、D14-14/15 轴 2 假阳性（深路由/块注释/注释内 `.ts` 提及误红）、D14-9 密钥模式集双实现、D14-11/12/13 重复样板、D14-22 报告 schema 未统一、D14-23 schema-drift job 仅 schedule + continue-on-error、D14-6 轴 1 字段级漂移未覆盖 | B10 | 低 | ⏸ **待执行**（均属门禁自身可靠性，可本地验证；按价值/风险逐个推进） |
