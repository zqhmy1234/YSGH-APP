...
| B10-i | 门禁域**剩余项**（**部分完成**）：D14-9 ✅（B10-l）、D14-11 ✅（B10-n）、D14-13 半 ✅（B10-o）；**仍待执行**：`D14-6`（轴 1 字段级漂移未覆盖）、`D14-12 残余`（subprocess 双实现）、`D14-22`（报告 schema 未统一）、`D14-23`（schema-drift job 仅 schedule + `continue-on-error`） | B10 | 低 | ⏸ **待执行（4 项）**（均属门禁自身可靠性，**可本地验证**，无需真机/Docker） |
