...
**产品决策项 → ✅ 四项已拍板并落地（2026-09-25 用户「全同意」＝按建议执行）**：

| # | 决策项 | 拍板 | 提交 |
|---|---|---|---|
| 10 | `D10-8` 生成态 LLM 输出是否过护栏 | **B + C**：规则层全量（四条链）+ 对话固定过一次 LLM 级 + 照片确定性抽样（默认 10%） | `ce71eb7` |
| 11 | `D10-9` 软敏感 fail-open | **C + D**：按"主动 vs 被动"分治（检测器没跑通 ⇒ 标"待复核"、主动提及路径跳过）+ 软话题词表扩容 | `58e2a71` |
| 12 | `D07-12` 画像写端点缺失 | **B 现在做**：真删除端点 + 客户端接线 + 弹窗文案改真话；A（改值/确认/锁定＋`D07-11`）排二期 | `56dac34` |
| 13 | `D02-9` 剩余（GIF / HEIF） | **以数据定**：`.gif` 不支持并删不可达条目；HEIF 族受理与 MIME **都保留**（实测 194 个 `.heic` 在用） | `3137132` + 本批 |

> 决策依据与选项对比保留在 `docs/决策台账.md` §5.11 + §5.11 拍板结果（唯一决策入口）。
