...
- **下一步 = 4b 修复批次**（代码工作，非文档）：
  ...
  - 批次2 **D-16/D-07/D-08(+D-22 建议并入待拍板 §5.7)** 语音链——D-16 ✅后端/端侧已修（fix/4b f1f8a3c，pytest 89 绿；复验随 R2）；验收＝未识别不再伪造"平静"（返 null+UI 不渲染）/ <5min 录音落 COS 可回放转写 / 转写失败段保留 wav+no_speech；
...
- **待用户拍板 3 项**（决策台账 §5）：部署就绪包确认 / 自定义分类目标页是否入 MVP / **D-22 并批确认（§5.7）**。另 D-09 遗留单腿暴露走法待拍板（tracker19 D-09 行注记）。08-29 已连销四项：交付文档同步授权、D-15='device'、取消=永久不再提示、**文案库基调（温和克制+禁区 10 项照单+默认库转正式 v1）**。
