...
> **定位**：`docs/lessons.md` 是程序化登记的**时间线台账**（131+ 条，勿手改结构）；本文件是它的**复盘索引**——同样族的坑只登记过不止一次，说明规则没进流程。每条给"族→根因→规矩→代表台账（日期·commit）"。新增教训后若属已有族，顺手在本表补代表条目。
...
- 台账规模：131 条（2026-08-19~08-28）。族1 占比 ~19%——**lint 类本不该占大头**，落实"先 ruff --fix 再门禁"应压到 0；族2+3 合计 ~20% 是 Android 端特性成本，已固化进两个 skill；族7 是纯流程债，本索引+决策台账即为对策。
