...
| D-05 | 01 Step2 | 客户端（横幅接线+队列） | 📋 待 Wave4 修，不阻塞：a) 横幅 emits 无宿主监听 → held 手动上传成功后不补端侧聚合/事件上云（contents 有、事件无）；b) drain 耗尽路径 held/failed 双登记无去重（计数虚增）。修复设计见 `evidence/ck01_step2_20260828_021530.log` 尾注 |【09-01 落地 fix/4b 381a210：uploader setOnUploadsComplete 钩子+index aggregateUploaded 共口（live/补传/手动/WiFi 四路同源聚合）；drain held∪failed 去重+holdRemaining 跨队列互斥——R2 复验】
...
| D-07 | 02 Step4 连带 | 客户端短录音入库路径+服务端管线 | 📋 移交 Wave4：<5min 短录音"确认入库"只提交转写文本、音频本地丢弃（voice.ts 仅 >5min 上传），但 process_content 对 voice 一律要求音频对象 → worker 实录 `ASR 失败: AUDIO_NOT_FOUND` → contents 标 failed（用户感知"转写失败"主因，短录音永久不可回放）。**05 R6 复现实锤（08-28 14:46）：用户 4 条真录音全部中招 failed 仅 text 幸存；设备缓存抽回原 wav 做情绪回放（evidence/ck05_emotion_replay_20260828.jsonl）**。叠加：同步转写 30s 硬超时（voice.ts:160），实测 76.9s 音频冷态 24.3s/热态 3.7s——>90s 冷态必超时。方向：短录音带音频入库或管线放行无音频 voice；转写异步化或超时随时长伸缩 |【09-01 落地 fix/4b 97eb795：短录音带音频入库（complete meta 携已审文本一次建内容+管线复听三守卫保用户文本）；分流 300s→250s 堵 4.2~5min 8MB 黑洞；转写超时随时长伸缩封顶 120s——R2 真机复验】
| D-08 | 02 Step5 补测连带 | 客户端转写失败处理 | 📋 移交 Wave4：自动停止/手动停止后 transcribeWav 失败（HTTP 非 200/超时）仅 toast 一次即重置空闲——**整段录音成孤儿**（无重试按钮、不落持久队列、UI 不留存 lastVoiceFile）。实例：04:54 的 60s 段（503），PC 直传回放可救（3.7s 出文）但端内无任何找回路径。方向：失败段入本地待发队列（同离线队列机制）+ 卡片保留"重试转写" |【09-01 落地 fix/4b 97eb795：失败段 prefs 持久队列+「重试转写」条+跨重启找回（不再孤儿）——R2 复验】
| D-09 | 02 Step5 补测连带 | 后端 ASR 双通道 | 📋 移交 Wave4（清单 05 前哨发现）：sensevoice 兜底通道 **ModuleNotFoundError**（uvicorn 实录×6），双通道实际单腿——funasr 通道网络抖动即整端点 503（04:54:46 实例）。funasr 主通道自身工作正常（当晚 200×6，热态 3.7s/冷态 24.3s）。方向：修 sensevoice 依赖安装或从降级链摘除并在健康检查暴露单腿状态。**08-28 15:40 依赖侧已修**：numpy2.5.2/scipy1.18/librosa/funasr-onnx/onnxruntime/modelscope 全链装齐，SenseVoice ONNX 经 ModelScope 首调下载（D 盘缓存），回放实证 emotion_source=sensevoice_local 双腿接通；遗留"健康检查暴露单腿"并入 D-16 处置。**08-29 设计冲突待拍板**：G2/R6#15 已把 /healthz 收敛为 {status:ok} 最小信息——单腿暴露改走哪条（鉴权 debug 端点 / 仅启动日志 / Sentry tag）→ **08-29 已拍板并落地：登录态 GET /api/v1/asr/channels**（fix/4b 9922e4a，只回布尔不泄漏 key） |
| D-10 | 03 环境段 | 客户端 utils/auth.ts:61,66 | 📋 Wave4 修（环境诱发实证）：`res.data as UTSJSONObject` 无守卫+未校验 statusCode/content-type——8000 端口被 HBuilderX httpServer 占用返回非 JSON 时直接 ClassCastException 崩环（06:56-07:08 实录多次）。另 auth.ts:61 TEMP code=dev-client-w3e 波尾须还原。方向：200+content-type 校验后 cast，否则可读错误"后端不可达" |【09-01 落地 fix/4b 2a0194b：auth.uts 双 cast 点收编 parseBody 安全解析（200 非 JSON→null 走可读失败分支）；TEMP dev-client-w3e 码已被 wrap1 规范化为 dev mock 口径无需还原——R2 冒烟复验】
...
| D-12 | 03→04 | 客户端 yishu-photo-watch | 🔒 关单+改进项：定性见 D-04（端侧无罪，w3k 实证）。保留观测性改进：readExifTaken dt==null 分支加告警日志（现静默回退链 EXIF→DATE_TAKEN→DATE_ADDED 全盲），Wave4 落实 |【09-01 落地 fix/4b 2a0194b：photo-watch 时间回退链两档 warn（EXIF 缺→DATE_TAKEN 可能扫描污染 / →DATE_ADDED 兜底可见）——随 D-03 型场景复验】
...
| D-14 | 04 R5 | 客户端 photo-watch 上传路径 | 📋 移交 Wave4（真缺陷·照片静默丢失）：隧道死时 uploadBatch 失败仅 toast——**游标已推进不回退、contents 无补传队列**（w3j 6 张永久丢失 @07:56；同窗 R4 50 张却经 pending drain 恢复，两路径行为不一致）。另：drain 重传 contents 跳过 handleBatch 端侧聚合（R4 轮 L1 长期为空）。方向：上传成功才推进游标 + 照片上传并入持久队列，重传后补端侧聚合 |【09-01 落地 fix/4b 381a210：游标维持查询层先推进（B-UT-3 防重触发风暴，08-28 风暴定性背书）；防丢落点=队列+钩子层闭环（failWholeBatch 兜底既有+drain 聚合链接通）——R2 断网重传复验】
| D-15 | 04 R5b | 端云事件语义 | 🔧 **已拍板待修（08-29 用户定 title_source='device'**，与 generated_by 对齐、避开 B3-5 背书保护 aggregate.py:184 误触）：移入 4b R1 随批次2处置。**08-29 R1-d 已修**（fix/4b 6f43e87：sync 路径 'device'+schemas/sql 注释同步；用户真编辑走服务端 edit 仍标 'user'）；复验=R2 端侧 L1 卡片落库标记核对 |
| D-16 | 05 R6 | 后端 ASR 情绪通道+客户端展示 | 🔧 已修待复验（R1-c f1f8a3c，08-29；真缺陷·伪造标签）：SenseVoice 增强失败（ModuleNotFoundError/模型缺失）时 transcriber.py:80 仅记 warning 并返回未增强结果，emotion 保持 models.py:66 + schemas/asr.py:21 双默认值"平静"、客户端 voice.ts:186 再 `?? '平静'` 兜底——**三层默认值把"没测出"联同伪装成"平静"，UI 无法区分**。老人情绪关怀场景高危：低落被标平静→notify.py:291 门控永不触发。方向：emotion_source=none 时 API 返回 null/「未识别」，UI source=none 不渲染情绪 chip。**08-29 落地**：三层默认值拆穿（AsrResult/SenseVoiceResult 默认=None、SenseVoice EMO_UNKNOWN→None、schema 可空、mock 不伪造、笑声提升兼容 None、端侧 null→'' chip 自然隐藏+主导情绪空提示守卫）；pytest 89 全绿（新增未测得回归+参数化加例）；遗留=端侧编译/真机复验随 R2（编译门被他窗 5.24 迁移卡）＋S2 校准 |
...
- **O-3（08-29 暗物质审计事件）**：主区旧脏按拍板③三重备份后清空（快照分支 `salvage/main-worktree-dirty-20260829`@2bad295 三父全量 + `.cowork-temp/salvage/` patch + 112 文件直拷），审出**两单「修了没入库」**：①D-02/D-03 修复组（勘误见 §4 对应行）②copy_library v1 基线资产（拍板引用文件未入库，7589775 补落含 notify 纯回退接线）。**残余他窗未提交工作**（sms providers+108、embedding O-1 dtype 修复+41、storage+67、cleanup_job+47、export API、orphan_scan worker、SuspectBadge 组件等）全存于 salvage 分支，物主可 `git checkout salvage/main-worktree-dirty-20260829 -- <path>` 单文件回捞；**该分支在认领清账前禁删**。证据目录 `scripts/realdevice/evidence/.gitignore`（「证据留本地」铁律）已恢复并提交入库防再误伤。**09-01 后记**：`.cowork-temp/salvage/` 暂存区被工作区清理 Agent 整删——untracked 112 文件与 tracked diff 均可从 salvage 分支再生（`git show 2bad295^3:<路径>` 取原件 / `git diff 2bad295^1 2bad295` 再生 patch），唯 rag/image.py 一份 1KB 预 stash 回滚 diff 永久损失（后被 dashscope 正式重写取代，风险可接受）。教训已登记。
- **O-4（09-01 跨窗合流波）**：wrap1-agentA2-ui-restore 17 枚经 `cea5025` 跨天真 merge 入 develop（老基线 677ea68；lessons/progress 双保留解冲）。本表连带效应：D-21 已入库（见 §4 行）；utils 21 文件 .ts→.uts；Vapor 启用（**R2 云打包口径以此为准**）；33 处 CSS gap 静默吞全修复（grep 零残留，本窗实测）。后续活体：①媒体票据 Valet Key（08-31 起）主区 8 文件未提交=勿碰名单；②08-31 全页面审查报告已抢救入库 `docs/audit_20260831_全页面系统性审查.md`+`审查总览.md`（detail hero 两 overlay 丢失层修复与否待 R2 目验）；③AI 对话页=contract-first mock，后端端点未实现→台账 §5.8 待拍板；④wrap1 worktree 仍活跃迭代 uvue_gen 管线（勿碰）。
- **O-5（09-01 暗物质第三例·U3 前提崩塌待重拍）**：R1 尾段实施时全端检索 `dismissCorrectionPrompt`——**提交侧零命中**；钩子与 D4 弹窗整条特性（3 次纠错→弹层→确认清零）只活在 stash 树（text_recorder.ts:288-300 + record.uvue 调用点，49bed24/2bad295），当年 A3「模拟器 E2E 已过」又跑在未提交树上（台账 5.3「hook 已就绪未接线」的前提=幻影）。RecordSheet 新结构亦无 D4 弹层。重接须整体回捞重构——**是否值得回炉请用户重拍**（选项：A 从 stash 移植回 D4 链+接线 dismiss；B 定性二期；C R2 后评估）。台账 5.3 行等他窗把 `docs/决策台账.md` 在途改动落账后再补注（本窗避混提交）。
