...
### 2026-08-29 16:40 · commit 9922e4a · ts=1787992815
- **错误**：D-02/D-03 两单 tracker 标「已修+真机复验过」，但 08-29 暗物质审计发现其修复代码（exif.py 全新文件+pipeline/register 改动）从未提交，只活在主工作区脏状态
- **根因**：多窗共享主工作区时代，验证跑在脏树上即宣布修毕；「已修」判定只查了行为没查 git 是否含该文件（新文件 exif.py 从未被 git add 不可能被 diff 审查发现）
- **修复**：从抢救快照 2bad295 提取五文件落 fix/4b（test_pipeline 24 绿含新 D-03 用例）；tracker D-02/D-03 行补勘误注
- **相关文件**：backend/app/services/exif.py
- **教训**：任何「已修」判定必须含 git ls-files 存在性检查——新文件未入库=修复不存在
...
### 2026-08-29 13:47 · commit e80176f · ts=1787982463
- **错误**：D-19 修复的工程根 client/AndroidManifest.xml 在 git status 不可见，差点以未跟踪状态丢失（且很可能解释了 08-28 Agent K 为何放弃正确位置改写进插件目录）
- **根因**：.gitignore 第121/127行为排除构建残留写了不锚定的通配规则 AndroidManifest.xml 和 res/，深度任意匹配——把 DCloud 官方定义的云打包合并点（工程根 manifest、nativeResources/android/res）一并吞掉；工具层静默吞文件+无告警
- **修复**：.gitignore 追加 !client/AndroidManifest.xml 与 !client/nativeResources/** 豁免链（目录先豁免才谈得上子树）；复验 check-ignore 三连返回1
- **相关文件**：.gitignore
- **教训**：排除生成物的通配 ignore 规则必须锚定路径（如 unpackage/ 前缀）；新增任何需要入库的文件后先跑 git check-ignore -v 自证未被吞，再看 git status

---

### 2026-08-29 13:22 · commit e80176f · ts=1787980929
- **错误**：develop@e80176f 纯基线编译不过（upload_protocol.ts putChunk .then 返回类型错误），但历史所有'编译通过'记录均为真——因为全部编译验证都跑在含他窗未提交修复的脏工作区上
- **根因**：共享主工作区 + 从不曾在干净 checkout 跑过冷编译：脏副本里的在途修复系统性掩盖基线损坏（与 warm cache 掩盖同族，本次由 fix/4b worktree --cleanCache 冷编译首次暴露）
- **修复**：fix/4b 内做同形最小热修（new Promise<number> 显式包装）保持 R1 滚动；merge 时以他窗 D-01 完整版为准；日后每波验收至少一次干净 worktree 冷编译
- **相关文件**：client/utils/upload_protocol.ts
- **教训**：'编译通过'的证据必须声明编译对象是脏工作区还是干净基线；共享工作区项目里，基线健康只能由独立 worktree 冷编译证明

---

### 2026-08-29 13:19 · commit e80176f · ts=1787980775
- **错误**：D-19 修复登记：FGS DataSyncService 在云包/正式包 manifest 中不存在，startService 必死；插件源码注释'标准基座自动回退'掩盖全基座失效
- **根因**：UTS 插件目录内 utssdk/app-android/AndroidManifest.xml（service+权限声明其实早已写好）不参与云端打包 manifest 合并——DCloud 官方合并点=项目根 AndroidManifest.xml + nativeResources/（HBuilderX≥3.6，ask.dcloud 答复 214927）；声明放错位 + 收尾波从未尸检正式包 manifest 的 <service> 节点，直到 07 清单 R7 才暴露
- **修复**：新建工程根 client/AndroidManifest.xml：service(uni.UNIYISHU001.DataSyncService, dataSync, exported=false) + 两插件权限全量归并；插件 manifest 头部加注'不参与云合并'防再信；R2 验收 aapt dump 尸检 + 端内 queryIntentServices 自检日志
- **相关文件**：client/AndroidManifest.xml; client/uni_modules/yishu-photo-watch/utssdk/app-android/AndroidManifest.xml
- **教训**：凡'写了声明就以为生效'的交付链路，必须在最终产物上做一次尸检正向验证；声明位置以平台官方合并机制为准，不凭其他生态惯例推断

---

### 2026-08-29 13:19 · commit e80176f · ts=1787980775
- **错误**：D-18 修复登记：WorkManager 探测恒 false，正式包/自定义基座全部误走 setInterval 假活降级，B5d 唤醒/周期/退避永不启用
- **根因**：探测用 ClassLoader.getResource('androidx/work/WorkManager.class')——Android 类全编译进 dex，.class 资源永不存在；且云打包 dex 尸检（work∈classes3）证明包内明明有类仍判降级。原注释'自定义基座 true'是未经正向对照验证的错误假设
- **修复**：改 marker asset 探测：client/nativeResources/android/assets/yishu/workmanager-marker.txt（工程级官方合并点）+ AssetManager.list() 无异常探测（目录不存在返回空数组，绕开 UTS catch 收不了 Exception 限制）；marker 与插件 dex 同源共存亡
- **相关文件**：client/uni_modules/yishu-background-tasks/utssdk/app-android/index.uts; client/nativeResources/android/assets/yishu/workmanager-marker.txt
- **教训**：能力探测必须用'随包交付且可尸检'的同源信号，并做过至少一次正向验证（预期 true 的构建上实测为 true）——恒 false 与恒 true 同样危险：静默降级无人察觉
