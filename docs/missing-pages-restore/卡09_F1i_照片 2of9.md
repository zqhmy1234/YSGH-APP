...
## 真值
- `.wt/missing-pages/uvue_gen/record_f1i_canvas.json`（390x844 · 37 节点 · 2026-09-02 导出，帧号 F1i）
- 关键文案抽校：9:41 / 照片 / 照片 · 2/9 / × / 分类 / AI 已预选 · 点标签可纠正 / 日常生活 / 家人与朋友

## 任务
RecordSheet 重写·照片 2/9 态（网格少图态，与 F1h/F1j/F1k 同构不同量）
...
## 交互接线
四张照片态帧（F1i/j/k）验证网格在不同数量下的自适应，一组一起做完

## 收尾（本卡）
1. 自审：对照 JSON 节点清单逐个核对（文案/颜色/间距/层级），孤儿 CSS 0，自评 ≥75 分。
2. 降级逐条追加 `_diff_ledger.md`（标注「卡09 F1i」）。
