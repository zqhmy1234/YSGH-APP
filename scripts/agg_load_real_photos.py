...
用法：
    python scripts/agg_load_real_photos.py   # 打印抽样统计（需 SCREENSHOT_DIR）
"""
...
def load_screenshots(directory: Path | None = SCREENSHOT_DIR) -> list[RawPhoto]:
    """扫描截图目录 → RawPhoto 列表（按时间升序，无 GPS，source="screenshot"）

    directory 为 None（未设 SCREENSHOT_DIR 环境变量）→ 返回空列表（场景15 跳过）。
    """
