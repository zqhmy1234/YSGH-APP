#!/usr/bin/env python3
"""事件聚合测试照片生成器（开发辅助脚本 · R1#14 从 app/services/event_aggregation 迁入）

生成 500 张模拟照片，覆盖 B3 十类分布矩阵 + 边界用例（供 run_validation /
test_event_aggregation_scripts 使用；B3-4/AGG-009/010/016 素材）。
...
用法：
    python scripts/agg_generate_photos.py        # 打印生成规模（默认 500 张）
    python -c "from scripts.agg_generate_photos import generate; ..."
"""
