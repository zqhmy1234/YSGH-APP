...
### §OO（2026-09-10 深夜，环境异常删除二次复发实锤 + 我修正自己一处判断 + bg 修复移交）

- **⚠️⚠️ 环境事故定性（同会话二次复发，模式锁死）**：对 client/ 做 git 写（`git rm <单个jar>` + .kt 字节级单行替换，两步都成功且 diff 干净）之后，`git status` 再现 **160+ tracked 文件 ` D`（全部 client/ 下：pages/static/icons/uni_modules/utils）+ unpackage/ 非跟踪产物（今日云包 vapor.apk、class 缓存）整体消失**。二次恢复：`git checkout HEAD -- client/` 单命令回齐（0 脏、98 关键文件在位）；unpackage 属可再生产物（下次编译/云打包自回）不追。**结论固化：本环境=对 client/ 任何 git 写操作都会触发批量异常删除（疑 AV/后台维护盯上 uni_modules 的 jar 路径或 client 子树写事件），不是我的命令问题（第一次也链式第二次单步，同结果）。任何 client/ 写操作移交新会话或峰宝手敲。**
