...
class Gen:
    ...
    def walk(self, node, parent, root=False):
        ...
        if t in VEC_TYPES or is_icon(node):
            ...
            _w = node.get('width', 20)
            _h = node.get('height', 20)
        ...
        if t == 'TEXT':
            ...
            for f in (node.get('fills') or []):
                if f['type'] == 'SOLID' and f.get('visible', True):
                    ...
                    par_has_glass = parent and any(
                        e.get('type') == 'BACKGROUND_BLUR' for e in (parent.get('effects') or []))
...
def convert(frame, slug):
    global ROOT_ID  # noqa: PLW0603 —— 单次转换的全局根 id，直改最简
    ROOT_ID = frame['id']
