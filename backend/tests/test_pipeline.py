...
class TestVoicePipeline:
    ...
    def test_voice_client_text_survives_retranscribe(self, db_user, monkeypatch, tmp_path):
        """D-07（09-01）：客户端入库带用户文本，管线复听只回填情绪不改文本（用户操作优先）。"""
        ...
        monkeypatch.setattr("app.services.pipeline._index_content", lambda *a: None)
        monkeypatch.setattr("app.services.pipeline._classify_content", lambda *a: None)
        from app.services.pipeline import process_content

        r = process_content(str(c.id))
        assert r["status"] == "done"
        db.refresh(c)
        assert c.text == "用户改过的西湖午后"
        assert (c.emotion or {}).get("source") == "sensevoice_local"
    ...
    def test_voice_queues_local_emotion_after_transcript(
        ...
        # 尾段还会合法入队 user 聚合任务（run_user_aggregation 带 user 实参）——
        # 本测试只验情绪任务入队形态：key 去重 + 实参随 *args
        assert (enrich_content_emotion, str(c.id), str(c.id)) in queued
        db.refresh(c)
        assert c.status == "done"
...
class TestPhotoPipeline:
    ...
    def test_photo_exif_backfills_taken_at(self, db_user, monkeypatch):
        """D-03：管线环节对分片注册路径回填 EXIF 拍摄时间（taken_at 权威修正）"""
        import io as _io
        ...
        ex = img.getexif()
        ex[36867] = "2026:08:22 08:00:00"
        buf = _io.BytesIO()
        img.save(buf, format="JPEG", exif=ex)
        get_storage_backend().put_object("photos/u/exif.jpg", buf.getvalue())

        c = _content(
            db,
            user.id,
            "photo",
            cos_key="photos/u/exif.jpg",
            taken_at=datetime(2026, 8, 28, 1, 9, tzinfo=timezone(timedelta(hours=8))),
        )
        monkeypatch.setattr(ds_mod, "image_caption", lambda k: "西湖边的荷花")
        from app.services.pipeline import process_content
        ...
        assert r["status"] == "done"
        db.refresh(c)
        expected = datetime(2026, 8, 22, 8, 0, 0, tzinfo=timezone(timedelta(hours=8)))
        assert c.taken_at == expected
        assert (c.extra or {}).get("exif_taken_at") == expected.isoformat()
