...
def asr_channels(_user: User = Depends(get_current_user)):
    """ASR 双通道单腿可见性（D-09 遗留 · 08-29 拍板：登录态 debug 端点）。

    /healthz 因 G2 最小信息纪律不承载通道细节——本端点仅回布尔与配置在否，
    绝不回 key/路径内容；供部署后快速判「双通道实际单腿」。
    """
    import importlib.util

    from app.core.config import settings

    sv_dir_raw = (settings.sensevoice_model_dir or "").strip()
    sv_dir = Path(sv_dir_raw) if sv_dir_raw else None
    return ApiResponse(data={
        "funasr": {
            "type": "remote",
            "ready": bool((settings.dashscope_api_key or "").strip()),
        },
        "sensevoice": {
            "type": "local",
            "runtime_installed": importlib.util.find_spec("funasr_onnx") is not None,
            "model_dir_configured": sv_dir is not None,
            "model_dir_exists": bool(sv_dir is not None and sv_dir.exists()),
        },
    })
