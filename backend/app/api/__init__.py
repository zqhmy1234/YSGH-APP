...
class RequestIdRoute(APIRoute):
    ...
    判定：content-type=application/json 且 body 为含"空 request_id 字段"的 dict
    （即 ApiResponse 成功体，兼容 response_model 返回的普通 Response 与直接 JSONResponse）；
    ...
    def get_route_handler(self):
        ...
        async def custom_handler(request: Request) -> Response:
            ...
            if not request_id:
                return response
            # 兼容两种响应形态：response_model 走 serialize_response 返回普通 Response，
            # 无 response_model 返回 JSONResponse——统一按 content-type + body 可解析判定。
