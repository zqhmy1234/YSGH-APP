...
def _echo_user_pref(res: dict, preferred_label: str | None) -> dict:
    """D-22（09-01）：带用户偏好时回显 agrees_with_user 供客户端参考显示。
    裁决结果永远是参考——用户操作优先（学习写入走 POST /corrections）。"""
    if preferred_label:
        res["preferred_label"] = preferred_label
        res["agrees_with_user"] = res.get("label") == preferred_label
    return res
...
def arbitrate_job(
    ...
    db = SessionLocal()
    try:
        return arbitrate(db, user_id, text, content_type, preferred_label)
    finally:
        db.close()
