    ...
    try:
        ...
        if not result.text.strip():
            if (content.text or "").strip():
                # D-07（09-01）：客户端文本权威在场，ASR 空结果非致命（保文本判 done，不重试轰炸）
                return "done"
            raise AsrError(
                "EMPTY_TRANSCRIPT",
                "ASR 返回空文本但未标记为空白语音",
        ...
        # D-07（09-01）：用户操作优先硬约束——客户端同步转写+用户编辑过的文本，
        # 管线复听绝不覆盖；仅长录音（落 COS 空文本入库）回填。
        ...
        _classify_content(db, content, content.text or result.text)
...
def _process_photo(db: Session, content: Content) -> None:
    ...
    # 0. D-03：EXIF 拍摄时间权威回填——分片上传注册路径不经 multipart 接口的
    #    _extract_exif_datetime，客户端 DATE_TAKEN 可能被扫描污染（2026-08-24 实测；
    #    2026-08-28 Wave3 真机复现）。此处以原图字节 EXIF 为准（子 IFD 36867 优先）。
    try:
        from app.services.exif import extract_exif_datetime

        exif_dt = None
        if tmp_file is not None and tmp_file.exists():
            exif_dt = extract_exif_datetime(tmp_file.read_bytes())
        elif image_path is not None and image_path.is_file():
            exif_dt = extract_exif_datetime(image_path.read_bytes())
        if exif_dt is not None and content.taken_at != exif_dt:
            patch_extra(
                content,
                exif_taken_at=exif_dt.isoformat(),
                prev_taken_at=(
                    content.taken_at.isoformat() if content.taken_at else None
                ),
            )
            content.taken_at = exif_dt
            logger.info(
                "EXIF 回填 taken_at content=%s %s -> %s",
                content.id, "corrected", exif_dt.isoformat(),
            )
    except Exception as exc:  # noqa: BLE001 —— EXIF 失败不否定照片处理
        logger.warning("EXIF 回填失败 content=%s: %s", content.id, exc)
        ...
        if emotion_pending:
            try:
                ...
                enqueue_unique(
                    ...
                    content_id,
                    # D-02：key 仅去重，实参必须重复传（enrich_content_emotion(content_id)）
                    content_id,
                    queue_name=QUEUE_LOW,
                    job_timeout=DEFAULT_JOB_TIMEOUT,
                )
