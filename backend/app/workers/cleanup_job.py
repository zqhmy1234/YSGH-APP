...
孤儿扫描已实现（Wave1 B1 收尾 §5.2.6）：独立模块 app/workers/orphan_scan.py，
本文件 main() 提供 --orphan-scan 入口（对齐 --older-than-days/--limit/--dry-run）：
  python -m app.workers.cleanup_job --orphan-scan --older-than-days 30 --limit 500
实现细节（三类孤儿/引用判定/失败安全/幂等/dry-run）见 orphan_scan.py 模块 docstring；
