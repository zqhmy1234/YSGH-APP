...
- **⚠️ 未闭环（如实标注）**：① **真机项未跑**（`adb devices` **无设备**）：B5c 的"真机上传"、B5d/B5.2e 的样式渲染视觉复验、**B5.2f 的 script 抽 composable（L-01/L-02 未闭环）** 均待设备接入补验；② **L-12 令牌收敛**（`design_tokens.json` 现 0 引用）**另立令牌波**；③ `favorites.uvue` 基线 974 vs 实际 975 为**先于本轮**的记账漂移（未上调基线，保持 WARN 如实提示）；④ 门禁域残余 `D14-12/22/23` **已于本轮收口**（见上），`D14-6` 判定已被 B10-g 覆盖 ⇒ 门禁自身可靠性**本轮清零**；⑤ 64 条功能与安全缺陷 → 用户已确认**另开「功能修复波」**（台账 §5.10 拍板 10.1）。

- **B11 余项（下批）**：`D07-14`（三套 dimensions 读路径，跨端）、`D09-10`（wechat status 枚举三方不一致；§5.10 已拍板 ORM 为权威 ⇒ 可据此对齐）；`D02-9` 移交功能波。