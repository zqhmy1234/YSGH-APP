...
- **⚠️ 未闭环（如实标注）**：① **真机项未跑**（`adb devices` **无设备**）：B5c 的"真机上传"、B5d/B5.2e 的样式渲染视觉复验、**B5.2f 的 script 抽 composable（L-01/L-02 未闭环）** 均待设备接入补验；② **L-12 令牌收敛**（`design_tokens.json` 现 0 引用）**另立令牌波**；③ `favorites.uvue` 基线 974 vs 实际 975 为**先于本轮**的记账漂移（未上调基线，保持 WARN 如实提示）；④ 门禁域低价值残余（D14-12/22/23）；⑤ 64 条功能与安全缺陷待拍板归属波次。